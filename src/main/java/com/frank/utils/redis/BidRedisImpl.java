...
import com.frank.utils.redis.config.RedisContext;
import com.frank.utils.redis.config.ShardedRedisPool;
import com.frank.utils.redis.serializer.SerializerFactory;
...
public class BidRedisImpl implements BidRedis {
    ...
    public Long expireAt(Map<String, Long> map) {
        ...
        try {
            ...
            returnResource(shardedJedis);
            logger.info("Redis expireAt(map) (" + map.toString() + ") took " + (System.currentTimeMillis() - begin) + " ms");
        }
        return sum;
    }
...
//			}
            ...
            return shardedJedis.zadd(SafeEncoder.encode(key), bScoreMembersNew);
